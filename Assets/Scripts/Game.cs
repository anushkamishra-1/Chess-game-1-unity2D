...
public class Game : MonoBehaviour
{
    ...
    //creating a private GameObject array
    //for all positions on the board
    //put [8,8] as [x,y] since the chessboard is 8x8
    ...
    public GameObject Create(string name, int x, int y)
    {
        ...
        cm.SetXBoard(x);
        cm.SetYBoard(y);
        cm.Activate(); //to set correct sprites
        return obj;
    }
    ...
    {
        gameOver = true;
        GameObject.FindGameObjectWithTag("YouWonText").GetComponent<Text>().enabled = true;
        GameObject.FindGameObjectWithTag("YouWonText").GetComponent<Text>().text
            = playerWin + (" is the winner!");
